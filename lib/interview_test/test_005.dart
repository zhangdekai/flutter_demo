/*
快速排序（Quicksort）是一种高效的排序算法，由 Tony Hoare 在 1960 年提出。它使用分治法（Divide and Conquer）策略，将待排序的数组递归地分成两个子数组，然后分别对这两个子数组进行排序，最终得到整个数组的有序结果。
算法步骤:
1. 从数组中选择一个元素作为 基准（pivot）。
2. 重新排序数组，所有元素比基准值小的摆放在基准前面，所有元素比基准值大的摆在基准的后面。这个过程称为 分区（partition）。
